  ...
  <div class="container">
    <table id="tblUsers">
      ...
      <tbody>
        ...
        >
          <td>
            <input type="checkbox" v-bind:id="user.id" v-bind:value="user.id" @click="idIntoArray(user.id)"/>
          </td>
    ...
    <form v-show="showForm" id="frmAddNewUser">
      ...
      <button type="submit" class="btn save" v-bind:disabled="!saveUser">Save User</button>
    </form>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    flipStatus(id) {
      ...
      } else this.users[index].status = "Active";
    }
  },
  computed: {
