...
public class Exercises {
	...
	public String[] list2Array(List<String> stringList) {
		// make a new array thats the size of the list given
		String [] stringArray = new String [stringList.size()];
		// runs as long as its less than the list size
	...
	public List<String> no4LetterWords(String[] stringArray) {
		...
		// running through each words in the array
		//
		for (String word: stringArray) {
			// checks word length 
			
			// does not equal 4 so adds to list
			// checks next word does not equal 4
			// checks last word and adds it
			if (word.length()!=4) {
	...
	public List<String> reverseList(List<String> stringList) {
		// put my list into a stack
		
		Stack<String> listStringsReverse = new Stack<String>();
		for (String current : stringList) {
			listStringsReverse.push(current);
			
		}	
		// move the stack into a list
		List<String> listToReturn = new ArrayList<String>();
		while (!listStringsReverse.empty()) {	
			listToReturn.add(listStringsReverse.pop());
		}
		// return the list
	...
	public Integer findLargest(List<Integer> integerList) {
		...
		//for ( Integer returnLargest : integerList);{
		//	current.
	...
	public List<Integer> oddOnly(Integer[] integerArray) {
        ...
        for(int i = 0; i < integerArray.length; i++) {
            if(integerArray[i] % 2 == 1) {
                currentList.add(integerArray[i]);
            }
        }
        return currentList;
	...
	public List<String> fizzBuzzList(Integer[] integerArray) {
		 ...
		 List<String> newCurrent = new ArrayList<String>();
	      ...
	      for(Integer i: integerArray) {
	    	  if (i %3 == 0 && i %5 == 0 ) {
	    		  newCurrent.add("FizzBuzz");
	    	  }else  if (i %3 == 0 ){
	    		  newCurrent.add("Fizz");
	    	  }else if (i %5 == 0){
	    		  newCurrent.add("Buzz");
	    		  
	    	  } else   newCurrent.add(i.toString());
	      }
