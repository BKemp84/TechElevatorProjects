...
public class Exercises {
	...
	// return unknown if animal not on list
	...
	public Map<String, Integer> robPeterToPayPaul(Map<String, Integer> peterPaul) {
		...
		int currentPeter = peterPaul.get("Peter");
		
		int currentPaul = peterPaul.get("Paul");
		
		if (currentPeter > 0 && currentPaul < 1000){
			    if     (currentPeter % 2 != 0) {
				currentPeter = currentPeter / 2;
				
				  currentPaul = currentPeter + currentPaul;
				  
				  currentPeter ++;
				  
			}  else if (currentPeter > 0 && currentPaul < 1000) {
				         currentPeter = currentPeter / 2;
		    ...
		    peterPaul.put("Paul", currentPaul);
		
		    peterPaul.put("Peter", currentPeter);
	...
	public Map<String, Integer> peterPaulPartnership(Map<String, Integer> peterPaul) {
		...
		//if peter has < 5000 and paul has >10000 then combine 1/4
			
		if ( currentPeter >= 5000 && currentPaul >= 10000) {
			
			  int peterPaulPartnershipPeter = currentPeter / 4;
			  // peter 1/4 sent to combine
			
			currentPeter = currentPeter - peterPaulPartnershipPeter;
			// paul sent to combine
			int peterPaulPartnershipPaul = currentPaul / 4;
			// what paul has if true
			currentPaul = currentPaul - peterPaulPartnershipPaul;
			// whats in combine
	...
	public Map<String, String> beginningAndEnding(String[] words) {
        ...
        Map<String, String> currentKey = new HashMap<String,String>();
		 // loop thru each
		for (String current : words){
					// put first letter in key last in value		
			currentKey.put(current.substring(0, 1),  current.substring(current.length() -1))	;		
		}
     ...
     Map<String, Integer> counts = new HashMap<String, Integer>();
		...
		for (String word : words) {
			...
			if(counts.containsKey(word)) {
				...
				// if it does not repeat stays at 1
			} else {
				counts.put(word, 1);
			}			
		}			
